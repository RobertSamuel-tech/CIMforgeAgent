...
const FieldExtraction = ({
    ...
    const [localCombinedRegex, setLocalCombinedRegex] = useState(null);
    ...
    useEffect(() => {
        ...
        if (aiFieldResults?.combined_regex !== undefined) {
            setLocalCombinedRegex(aiFieldResults.combined_regex);
        }
    }, [aiFieldResults]);
    ...
    const handleAcceptAiFields = (useCombinedRegex = false) => {
        ...
        setAllFields(updatedFields);
        const extractionRegex = useCombinedRegex && localCombinedRegex ?
            localCombinedRegex : null;
    ...
    const handleAcceptCombinedRegex = () => {
        if (!localCombinedRegex) return;

        // Extract fields from the combined regex
        const fields = [];
        try {
            let jsRegex = localCombinedRegex.replace(/\(\?P</g, '\\(?<');
            const regex = new RegExp(jsRegex, 'g');
        ...
        onFieldsExtracted(updatedFields, localCombinedRegex, {
            timeFormat: aiFieldResults.time_format,
            timePrefix: aiFieldResults.time_prefix || '',
            maxTimestampLookahead: aiFieldResults.max_timestamp_lookahead || '25'
        });
                                ...
                                {localCombinedRegex ? (
                                    <>
                                        ...
                                        <StyledBox marginBottom={16}>
                                            ...
                                            <StyledTextArea
                                                value={localCombinedRegex}
                                                onChange={(e, { value }) => setLocalCombinedRegex(value)}
                                                rows={4}
                                            />
                                            <Button
                                                appearance="secondary"
                                                size="small"
                                                onClick={() => handlePreviewCombinedRegex(localCombinedRegex)}
                                                style={{ marginRight: 8 }}
                                            >
                                                Preview Extraction
                                            </Button>
                                            {combinedRegexPreview && (
                                                <StyledPreviewBox>
                                                    ...
                                                    <pre style={{ fontSize: 12, marginTop: 4 }}>{JSON.stringify(combinedRegexPreview, null, 2)}</pre>
                                                </StyledPreviewBox>
                                            )}
                                        </StyledBox>
                                        ...
                                        <StyledBox marginBottom={16}>
                                            ...
                                            <StyledSampleDataBox
                                                dangerouslySetInnerHTML={{
                                                    __html: highlightFieldsInSample(sampleData, localCombinedRegex)
                                                }}
                                            />
                                        </StyledBox>
